...
| `GRACEFUL_SHUTDOWN_TIMEOUT` | `8` | Total budget in seconds for stopping all running pipeline runs when the orchestrator shuts down: Docker containers are stopped, Kubernetes jobs are deleted — in both cases in parallel and within this budget. On Kubernetes every API call additionally gets half of the budget as a client-side timeout, because the Kubernetes client has no default timeout and an unreachable API server would otherwise block forever. Must stay below the orchestrator's own stop grace period (`stop_grace_period` in docker-compose, `terminationGracePeriodSeconds` in Kubernetes) — otherwise the process is killed mid-shutdown. Runs that are not finished in time stay `RUNNING` on purpose and are resolved by the zombie reconciliation on the next start (on Kubernetes, a run whose job no longer exists becomes `INTERRUPTED`). |
