...
| `KUBERNETES_JOB_SERVICE_ACCOUNT` | *Empty* (namespace `default` SA) | ServiceAccount for pipeline Job pods. Pipeline pods never get a ServiceAccount token regardless of this value (`automountServiceAccountToken: false` — a mounted token would be an ambient cluster credential for untrusted pipeline code, see [Security Rollout](SECURITY-ROLLOUT.md), section 2b). This option additionally detaches the pods from the `default` SA identity; recommended value `fastflow-runner`, the binding-less SA shipped in `k8s/rbac-kubernetes-executor.yaml`. **Apply that manifest before setting this:** if the SA is missing in the namespace, admission never creates the pod, the Job stays pod-less and the run hangs in `RUNNING` (no deadline when `CONTAINER_TIMEOUT` is unset). |
