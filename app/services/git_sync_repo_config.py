"""
Git Sync Repository Config: URL + PAT (DB + Env).

Konfiguration für Git-Sync per Repository-URL und optionalem Personal Access Token.
Werte können in der Sync-UI (DB) oder per Umgebungsvariablen (GIT_REPO_URL, GIT_SYNC_TOKEN) gesetzt werden.
...
def get_sync_repo_config(session: Session) -> Optional[Dict[str, Any]]:
    ...
    Returns:
        Dict mit repo_url, token (entschlüsselt oder None), branch.
